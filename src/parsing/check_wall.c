...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/08 22:09:06 by iidkhebb          #+#    #+#             */
/*   Updated: 2022/11/08 22:10:58 by iidkhebb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	deep_check_walls_one(char **tab, int i, int j)
{
	if (deepcheck_extra(tab, i, j) == 0)
		return (0);
	if (ft_strchr("D", tab[i][j]) && (tab[i][j + 1] != '1' || \
		tab[i][j - 1] != '1') && (tab[i + 1][j] != '1' || tab[i - 1][j] != '1'))
	{
		return (0);
	}
	else if (ft_strchr("D", tab[i][j]) && (tab[i + 1][j] != '1' || \
		tab[i - 1][j] != '1') && (tab[i][j + 1] != '1' || tab[i][j - 1] != '1'))
	{
		return (0);
	}
	if (tab[i][j] == '0' && (tab[i][j + 1] == ' ' || tab[i][j - 1] == ' '))
		return (0);
	if (ft_strchr(PLAYER_POS, tab[i][j]) && (tab[i][j + 1] == '1' && \
		tab[i][j - 1] == '1') && (tab[i + 1][j] == '1' && tab[i - 1][j] == '1'))
	{
...
int	check_walls(char **tab)
{
	...
	while (tab[i])
	{
		...
		if (!valid_map(tab[i]))
			return (0);
		if (!check_first_line(tab))
			return (0);
		else if (i == last && !check_last_line(tab, last))
			return (0);
		else if (i != 0 && i != last && !check_inner_walls(tab, i))
			return (0);
		i++;
	}
	if (p_count != 1)
